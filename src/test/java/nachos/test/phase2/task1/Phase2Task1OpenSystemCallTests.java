...
/**
 * Tests for open system call.
 */
public class Phase2Task1OpenSystemCallTests extends NachosUserProgramTestsSuite {
    public Phase2Task1OpenSystemCallTests() {
        super("phase2/phase2.round.robin.conf");
    }

    /**
     * Tests if calling open on existing file opens file as expected.
     */
    @Test
    public void testIfOpenOpensExistingFile() throws Throwable {
        ...
        List<SyscallCallRecord> records = runUserProgram("call_syscall", new String[]{"open", file}).get(UserProcess.rootPid);

        threadAssertLastExitSyscallStatusEquals(0, records);
        threadAssertEquals(2, records.size());

        threadAssertEquals(UserProcess.syscallOpen, records.get(0).syscall);
    ...
    /**
     * Tests if calling open returns -1 when provided file name exceeds
     * max string length (256), because readVirtualMemoryString() is
     * expected to return null.
     */
    @Test
    public void testIfOpenFailsWhenProvidedFilenameExceedsMaxStringLength() throws Throwable {
    ...
    /**
     * Tests if calling open on absent file returns -1.
     */
    @Test
    public void testIfOpenFailsWhenTryingToOpenAbsentFile() throws Throwable {
    ...
    /**
     * Tests if open returns -1 when no free file descriptor exists.
     * <p>
     * See test_open_1.c for more detailed description on how it is done.
     */
    @Test
    public void testIfOpenFailsWhenNoFreeFileDescriptorExists() throws Throwable {
        final String file = "test_file.txt";
        threadAssertTrue(doesFileExistsInNachosHomeDirectory(file));
        final String fileContents = readContentsOfFileInNachosHomeDirectory(file);

        List<SyscallCallRecord> records = runUserProgram("test_open_1", new String[]{file}).get(UserProcess.rootPid);

        threadAssertLastExitSyscallStatusEquals(0, records);
        threadAssertEquals(16, records.size());

        for ( int i = 0; i < 14; i++ ) {
            threadAssertEquals(UserProcess.syscallOpen, records.get(i).syscall);
            threadAssertNotEquals(-1, records.get(i).valueReturnedBySyscall);
        }

        threadAssertEquals(UserProcess.syscallOpen, records.get(14).syscall);
    ...
    /**
     * Tests if open returns -1 when called on file register for deletion.
     * <p>
     * See test_open_2.c for more detailed description on how it is done.
     * <p>
     * N.B. Depends on another syscalls so it's possible, that this test will
     * fail even if handleOpen implementation is correct!
     * <p>
     * N.B.2 Depends on working multiprogramming support!
     */
    @Test
    public void testIfOpenFailsWhenCalledOnFileRegisteredForDeletion() throws Throwable {
        final String file = "test_file.txt";
        threadAssertTrue(doesFileExistsInNachosHomeDirectory(file));

        HashMap<Integer, ArrayList<SyscallCallRecord>> recordsMap = runUserProgram("test_open_2", new String[]{"open", file});
        ...
        threadAssertEquals(UserProcess.syscallOpen, childRecords.get(1).syscall);
    ...
    /**
     * Tests if open returns -1 when StubFileSystem open files count exceeds.
     * <p>
     * See test_open_3.c for more detailed description on how it is done.
     * <p>
     * N.B. Depends on another syscalls so it's possible, that this test will
     * fail even if handleOpen implementation is correct!
     * <p>
     ...
     */
    @Test
    public void testIfOpenFailsWhenStubFileSystemOpenCountExceeds() throws Throwable {
        final String file = "test_file.txt";
        threadAssertTrue(doesFileExistsInNachosHomeDirectory(file));
        final String fileContents = readContentsOfFileInNachosHomeDirectory(file);

        HashMap<Integer, ArrayList<SyscallCallRecord>> recordsMap = runUserProgram("test_open_3", new String[]{"first", file});
        ...
        for ( int i = 0; i < 14; i++ ) {
            threadAssertEquals(UserProcess.syscallOpen, rootRecords.get(i).syscall);
        ...
        threadAssertEquals(UserProcess.syscallOpen, childRecords.get(0).syscall);
