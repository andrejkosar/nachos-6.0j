...
public class CoffSection {
     ...
     * @param coff         the coff loader instance
     * @param headerOffset the offset of the section header in the
     *                     executable.
     * @throws CoffLoadingException if an error occurs.
     */
    public CoffSection(OpenFile file, Coff coff, int headerOffset) throws CoffLoadingException {
        this.file = file;
        this.coff = coff;
        ...
        int vaddr = Lib.bytesToInt(buf, 12, ByteOrder.LITTLE_ENDIAN);
        size = Lib.bytesToInt(buf, 16, ByteOrder.LITTLE_ENDIAN);
        contentOffset = Lib.bytesToInt(buf, 20, ByteOrder.LITTLE_ENDIAN);
        int numRelocations = Lib.bytesToUnsignedShort(buf, 32, ByteOrder.LITTLE_ENDIAN);
        int flags = Lib.bytesToInt(buf, 36, ByteOrder.LITTLE_ENDIAN);
        ...
        if ( vaddr % Processor.pageSize != 0
            ...
            Lib.debug(dbgCoffSection, "\tinvalid section addresses: vaddr=" + vaddr + " size=" + size + " contentOffset=" + contentOffset);
            throw new CoffLoadingException();
        }
