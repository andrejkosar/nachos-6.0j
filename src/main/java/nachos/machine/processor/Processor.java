...
public final class Processor {
    ...
    public Processor(Privilege privilege, int numPhysPages) {
        ...
        Class<? extends Kernel> clsKernel = Lib.loadClass(Config.getString("Kernel.kernel"), Kernel.class);
        Class<? extends VMKernel> clsVMKernel = Lib.tryLoadClass("nachos.vm.VMKernel", VMKernel.class);
    ...
    public void run() {
        ...
        Machine.nachosRuntimeRecorder().runProcessor(privilege);
    ...
    private int readMem(int vaddr, int size) throws MipsException {
        ...
        int value = Lib.bytesToInt(mainMemory, translate(vaddr, size, false), size, ByteOrder.LITTLE_ENDIAN);
    ...
    private void writeMem(int vaddr, int size, int value)
        ...
        Lib.bytesFromInt(mainMemory, translate(vaddr, size, true), size, value, ByteOrder.LITTLE_ENDIAN);
    }
    ...
    private void handleMipsException(MipsException e) {
        writeRegister(regCause, e.cause);

        if ( e.hasBadVAddr ) {
            writeRegister(regBadVAddr, e.badVAddr);
        }

        if ( Lib.test(dbgDisassemble) || Lib.test(dbgFullDisassemble) ) {
            System.out.println("exception: " + exceptionNames[e.cause]);
        }
        ...
        // recorder might not want kernel to know about this exception
        if ( !Machine.nachosRuntimeRecorder().exceptionHandler(privilege) ) {
            return;
        }
